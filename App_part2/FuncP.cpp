...
// DEBUG:NOTE not worth to parallelize, array of size 4~
...
// DEBUG:NOTE:DELETEME
//      To parallelize, we will need to rewrite the function of index.
//       - Currently : we increment for each loop turn
//       - What we need : calculate the index based on the other values (doable, but chiant)
...
int *findLocalMedoidCandidate(int *mat_distance_fragment, int k, int nb_nodes, int nb_lignes_fragment)
{
    ...
    // DEBUG:DELETEME:NOTE cant be omp para for trivially because of the dirty trick we use
    for (int i = 0; i < k;)
    {
        int used = 0;
...
#pragma omp parallel for shared(mat_distance_fragment, index_locaux)
        ...
        for (int i = 0; i < k; ++i)
        {
            ...
            for (int j = 0; j < nb_nodes; ++j)
            {
                ...
                if (!is_in(copy, j, k) && !is_in(index_locaux, j, nb_lignes_fragment))
                { // && !is_in index_locaux
                    copy[i] = j;
